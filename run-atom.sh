...
BLOG_ARGS=${1:-/usr/blog-0.10/example/burglary.blog}
if [ $# -lt 1 ]; then
    echo "Usage: "
    echo "  ${0} <blog file options and path>"
    echo "e.g."
    echo "  ${0} /usr/blog-0.10/example/burglary.blog"
    echo "(note: blog is installed in /usr/blog-0.10 folder inside container)"
    echo "  ${0} -n 1000000 /usr/blog-0.10/example/example/burglary.blog"
    echo ""
    echo "The following will run a simple Hidden Markov Model for genetic sequences. It uses particle filtering algorithm to make inference."
    echo "  ${0} dblog example/hmm.dblog"
else
    BLOG_ARGS=$*
fi
